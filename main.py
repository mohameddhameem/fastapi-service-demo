...
# Define a route for displaying a form
@app.get("/updated_form")
async def form(request: Request):
    # Render the form.html template
    return templates.TemplateResponse("updated_form.html", {"request": request})


# Define a route for submitting the form
@app.post("/submit_form")
async def form_post(request: Request, name: str = Form(...), address: str = Form(...)):
    # Validate if name and address are retrieved from the request
    if name and address:
        # Return the submitted form data
        return {"name": name, "address": address}
    else:
        # Return an error message if name or address is missing
        return {"error": "Name and address are required. Validate the form."}
